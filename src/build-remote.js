  ...
  if (!args.customEntry && webpackConfig.entry) {
    if (!Array.isArray(webpackConfig.entry)) {
      Object.keys(webpackConfig.entry).forEach((key) => {
        ...
        let template = join(modulePath, 'index-remote.html');
        if (fs.existsSync(template)) {
          pluginConfig.template = template;
        } else {
          template = join(webpackConfig.baseDir, 'src', 'template-remote.html');
          if (fs.existsSync(template)) {
            pluginConfig.template = template;
          }
        }
        ...
        // if (dllReference) {
        //   pluginConfig.dllReference = dllReference;
        // }
...
export default function build(args, callback) {
  ...
  function doneHandler(err, stats) {
    ...
    copy(join(fileOutputPath, '*'), join(args.cwd, 'dist', '_remote'), (err1) => {
